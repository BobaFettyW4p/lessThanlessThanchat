...
inline std::vector<std::byte> to_bytes(const ChatLine& m) {
  ...
  put(out, m.room); put(out, m.user); put(out, m.text);
  return out;
}
...
inline ChatLine from_bytes<ChatLine>(std::span<const std::byte> s) {
  auto get = [](std::string& out, std::span<const std::byte>& sp) {
    if (sp.size() < 4) throw std::runtime_error("bad frame");
    uint32_t n;
    std::memcpy(&n, sp.data(), 4);
    sp = sp.subspan(4);
    if (sp.size() < n) throw std::runtime_error("bad frame");
    out.assign(reinterpret_cast<const char*>(sp.data()), n);
    sp = sp.subspan(n);
  };
  ChatLine m;
  auto sp = s;
  get(m.room, sp); get(m.user, sp); get(m.text, sp);
  return m;
}
