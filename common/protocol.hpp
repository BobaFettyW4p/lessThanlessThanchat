...
std::vector<std::byte> make_frame(const T& msg) {
  ...
  FrameHeader h{
    .len_be   = to_be32(static_cast<uint32_t>(payload.size())),
...
inline uint32_t header_len(const FrameHeader& h)  { return from_be32(h.len_be); }
inline uint16_t header_type(const FrameHeader& h) { return from_be16(h.type_be); }
