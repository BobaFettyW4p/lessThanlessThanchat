...
void Session::start(){
  ...
  if (auto h = hub_.lock()) {
    h->join(room_, shared_from_this(), [this](const std::vector<std::byte>& data) {
      send_raw(data);
    });
  }
  async_read_header();
    ...
    [this, self, type](std::error_code ec, std::size_t){
      ...
      try {
        if (type == ChatLine::type_id) {
          ChatLine msg = from_bytes<ChatLine>(std::span<const std::byte>(payload_.data(), payload_.size()));
          // Basic broadcast to room (room_, user_ ignored for now; agent can enhance)
          if (auto h = hub_.lock()) {
