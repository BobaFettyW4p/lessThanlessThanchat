...
TEST_F(ProtocolTest, MakeFrameBasic) {
    ...
    EXPECT_EQ(header_len(header), msg.room.size() + msg.user.size() + msg.text.size() + 12); // 3 * 4 bytes for lengths
}
...
TEST_F(ProtocolTest, MakeFrameEmptyMessage) {
    ...
    // Check frame size
    EXPECT_EQ(frame.size(), sizeof(FrameHeader) + 12); // 3 * 4 bytes for empty string lengths
...
TEST_F(ProtocolTest, MakeFrameLargeMessage) {
    ...
    // Check frame size
    EXPECT_EQ(frame.size(), sizeof(FrameHeader) + 12 + 3000); // 3 * 4 bytes for lengths + 3000 chars
