...
TEST_F(MessagesTest, DeserializationErrorHandling) {
    ...
    std::vector<std::byte> bad_length_data = {
        std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0xFF}, // length = 255
        std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, std::byte{0x00}, // user length = 0
...
TEST_F(MessagesTest, SerializationConsistency) {
    // Test that multiple serializations of the same message produce the same result
    ChatLine msg;
    msg.room = "consistent";
    msg.user = "test";
    msg.text = "message";
